...
/* ---- configurable set-associative write-back cache ---- */
typedef struct {
    uint32_t nsets, ways, lsz, lshift;
    uint32_t *tag;      /* [set*ways+way]  valid<<31 | tag */
    uint8_t  *dirty;
    uint8_t  *age;      /* LRU counter */
    uint64_t hit, miss, wb;   /* wb = dirty line evictions */
} cache_t;
static cache_t IC, DC;
static int unified = 0;
static uint64_t fmiss, lmiss, smiss;
static int wb_policy = 1;   /* 1 = write-back+allocate, 0 = write-through+no-allocate */

static void cache_init(cache_t *c, uint32_t kb, uint32_t lsz, uint32_t ways) {
    c->lsz=lsz; c->ways=ways;
    c->nsets = (kb*1024)/(lsz*ways);
    c->lshift = 0; while ((1u<<c->lshift) < lsz) c->lshift++;
    c->tag  = calloc((size_t)c->nsets*ways, 4);
    c->dirty= calloc((size_t)c->nsets*ways, 1);
    c->age  = calloc((size_t)c->nsets*ways, 1);
    c->hit=c->miss=c->wb=0;
}
/* is_write only meaningful for DC */
static void cache_acc(cache_t *c, uint32_t a, int is_write) {
    uint32_t line = a >> c->lshift;
    uint32_t set  = line % c->nsets;
    uint32_t tag  = (line / c->nsets) | 0x80000000u;
    uint32_t base = set * c->ways;
    for (uint32_t w=0; w<c->ways; w++) {
        if (c->tag[base+w] == tag) {
            c->hit++;
            for (uint32_t k=0;k<c->ways;k++) if (c->age[base+k] < 255) c->age[base+k]++;
            c->age[base+w]=0;
            if (is_write && wb_policy) c->dirty[base+w]=1;
            return;
        }
    }
    c->miss++;
    if (!wb_policy && is_write) return;            /* write-through, no allocate */
    uint32_t victim=0, oldest=0;
    for (uint32_t w=0; w<c->ways; w++) {
        if (!(c->tag[base+w] & 0x80000000u)) { victim=w; goto place; }
        if (c->age[base+w] >= oldest) { oldest=c->age[base+w]; victim=w; }
    }
    if (c->dirty[base+victim]) c->wb++;
place:
    c->tag[base+victim]=tag; c->dirty[base+victim]= (is_write && wb_policy);
    for (uint32_t k=0;k<c->ways;k++) if (c->age[base+k] < 255) c->age[base+k]++;
    c->age[base+victim]=0;
}
static uint64_t last_icm,last_dcm,last_dcwb,last_ich,last_dch;
static inline void acc(cache_t *c, uint32_t a, int kind){ uint64_t m=c->miss; cache_acc(c,a,kind==2); if(c->miss!=m){ if(kind==1) fmiss++; else if(kind==2) smiss++; else lmiss++; } }

static inline uint8_t *resolve(uint32_t a, int is_fetch, int sz) {
    if (a >= RAM_BASE && a < RAM_BASE+RAM_SIZE) {
        if(is_fetch==1){f_ram++; acc(unified?&DC:&IC,a,1);} else {d_ram++; acc(&DC,a,is_fetch);}
        return ram + (a-RAM_BASE); }
    if (a >= ROM_BASE && a < ROM_BASE+ROM_SIZE) {
        if(is_fetch==1){f_rom++; cache_acc(&IC,a,0);} else d_rom++;
        return rom + (a-ROM_BASE); }
    if (a >= WAD_BASE && a < WAD_BASE+wad_size){ d_wad++; return wad + (a-WAD_BASE); }
    if (a >= BRM_BASE && a < BRM_BASE+BRM_SIZE){ if(is_fetch==1) f_rom++; else d_ram++; return brm + (a-BRM_BASE); }
    return NULL;
}
static void report(const char *tag) {
    uint64_t i=n_instr-last_instr, l=n_load-last_load, s=n_store-last_store;
    printf("%-8s instr=%-10llu | Dacc=%-8llu Dmiss=%-8llu Dwb=%-8llu | Imiss=%-8llu\n",
        tag,(unsigned long long)i,
        (unsigned long long)((d_rom-last_drom)+(d_ram-last_dram)),
        (unsigned long long)(DC.miss-last_dcm),(unsigned long long)(DC.wb-last_dcwb),
        (unsigned long long)(IC.miss-last_icm));
    last_icm=IC.miss; last_dcm=DC.miss; last_dcwb=DC.wb;
    fflush(stdout);
...
static uint32_t load(uint32_t a, int sz, int sext) {
    if (a == 0x84000000u) return (uint32_t)n_instr;
    n_load++;
...
static void store(uint32_t a, uint32_t v, int sz) {
    if (a == 0x84000004u) {
        printf("SUMMARY instr=%llu loads=%llu stores=%llu fmiss=%llu lmiss=%llu smiss=%llu wb=%llu Imiss_split=%llu\n",
          (unsigned long long)n_instr,(unsigned long long)n_load,(unsigned long long)n_store,
          (unsigned long long)fmiss,(unsigned long long)lmiss,(unsigned long long)smiss,
          (unsigned long long)DC.wb,(unsigned long long)IC.miss);
        exit(0);
    }
    n_store++;
...
int main(int argc, char **argv) {
    ...
    uint32_t dkb = argc>4?atoi(argv[4]):8, dls = argc>5?atoi(argv[5]):32, dw = argc>6?atoi(argv[6]):1;
    if (argc>7) wb_policy = atoi(argv[7]);
    if (argc>8) unified = atoi(argv[8]);
    cache_init(&IC, 8, 32, 1);
    cache_init(&DC, dkb, dls, dw);
    fprintf(stderr,"[D$ %u KB, %u B lines, %u-way, %s]\n", dkb, dls, dw, wb_policy?"write-back":"write-through");
    rom=calloc(1,ROM_SIZE); ram=calloc(1,RAM_SIZE); brm=calloc(1,BRM_SIZE);
    int wf=-1; struct stat st; st.st_size=0; if (strcmp(argv[2],"-")) { wf=open(argv[2],O_RDONLY); fstat(wf,&st); } wad_size=st.st_size;
    if (wad_size) wad=mmap(NULL,wad_size,PROT_READ,MAP_PRIVATE,wf,0);
